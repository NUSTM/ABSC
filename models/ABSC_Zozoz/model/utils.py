...
def load_inputs_rdmn(inputs_file, word2id_file, aspect2id_file, max_len, encoding='utf-8'):
    if type(word2id_file) is str:
        word2id = load_word_id_mapping(word2id_file)
    else:
        word2id = word2id_file
    print 'load word2id done!'

    if type(aspect2id_file) is str:
        aspect2id = load_aspect2id(aspect2id_file)
    else:
        aspect2id = aspect2id_file
    print 'load aspect2id done!'

    x, y, sen_len, aspects, position = [], [], [], [], []
    for line in open(inputs_file):
        aspect, polarity, sentence = line.strip().split('||')

        aspects.append(aspect2id.get(aspect, 0))

        y.append(int(polarity))
        ...
        for word in sentence.split():
            ...
            if word in word2id:
                ...
                pp.append(ind)
        x.append(xx + [0] * (max_len - len(xx)))
        position.append(pp + [0] * (max_len - len(pp)))
        sen_len.append(len(xx))
    y = change_y_to_onehot(y)
    return np.asarray(x), np.asarray(y), np.asarray(sen_len), np.asarray(aspects), np.asarray(position)


def load_inputs_twitter_lcr(input_file, word_id_file, sentence_len, type_='', is_r=True, target_len=10, encoding='utf8'):
    if type(word_id_file) is str:
        word_to_id = load_word_id_mapping(word_id_file)
    else:
        word_to_id = word_id_file
